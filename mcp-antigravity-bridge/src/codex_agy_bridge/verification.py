...
class VerificationEvidence:
    ...
    # VerificationEvidence is controller-generated by default for backwards
    # compatibility; worker reports must explicitly set this to False.
    ...
    def from_dict(cls, data: dict[str, Any]) -> VerificationEvidence:
        ...
        return cls(
            ...
            independently_verified=bool(data.get("independently_verified", True)),
        )
...
def attest_source_provenance(
    ...
    attestation_script = (
        ...
        "try:\n"
        "    search_entries = []\n"
        "    search_entries.extend(p for p in os.environ.get('PYTHONPATH', '').split(os.pathsep) if p)\n"
        "    search_entries.extend(sys.path)\n"
        "    for entry in search_entries:\n"
        "        package_candidate = os.path.join(entry, 'codex_agy_bridge', '__init__.py')\n"
        "        if os.path.isfile(package_candidate):\n"
        "            res['package_file'] = os.path.abspath(package_candidate)\n"
        "            server_candidate = os.path.join(entry, 'codex_agy_bridge', 'server.py')\n"
        "            if os.path.isfile(server_candidate):\n"
        "                res['server_file'] = os.path.abspath(server_candidate)\n"
        "            break\n"
...
def evaluate_auto_commit_policy(
    ...
    if contract.risk_class in (RiskClass.HIGH, RiskClass.PRODUCTION):
        reasons.append("Task risk class is PRODUCTION; automated commits require human review")
    elif contract.risk_class in (RiskClass.MEDIUM, RiskClass.DESTRUCTIVE):
    ...
    if not evidence.independently_verified:
        reasons.append("Worker-reported evidence is a candidate and is not independent verification")
